...
import numpy as np
# import pandas as pd   # don't actually need this, astropy imports it already
import astropy.table as table
import tensorflow as tf

#uf_columns = ['time','crsv','crsu','amp_sf','av1','av2','av3','au1','au2','au3','rawx','rawy',
#    'chipx','chipy','tdetx','tdety','detx','dety','x','y','pha','pi','sumamps','chip_id','status']
#cl_columns = ['time','tg_r','tg_d','chipx','chipy','tdetx','tdety','detx','dety','x','y',
#    'chip_id','pha','pi','tg_m','tg_lam','tg_mlam','tg_srcid','tg_part','tg_smap','status']
...
class EventClassifier():
    ...
    Parameters
    ----------
    uf_file: string (default 'evt0')
    unfiltered (good+bad events) event file
    cl_file: string (default 'evt2')
    filtered (only good events) event file
    ...
    status_name: string, optional (default 'status')
    name of "status" column (bitmask for event status)
    batch_size: integer, optional (default 32)
    batch size for tensorflow datasets
    dense_layers: tuple of integers (default (128,))
    tuple of dense layer sizes, each becomes a tf.keras.layers.Dense(N) layer in the model
    load_and_initialize: bool (default True)
    run the process_events() and compile_model() steps on initialization

    Methods
    -------
    process_events(self,expand_status=True):
    Load and process the input FITS files (uf_file and cl_file) into self.data_frame
    ...
    Split self.data_frame into training, validation, and testing Tensorflow datasets
...
#   expand all of them?
    def load_fits(self, fname, status_name='status', verbose=0):
        ...
        if verbose:
            print('Converting status bits in {}'.format(fname))
        ...
        status_columns = [table.Column(col, name) for col, name
                          in zip(np.int16(fits_file[status_name].swapaxes(0, 1)),
                                 status_column_names)]
...
# self.cl_table
    def load_events(self, uf_file, cl_file, status_name='status'):
        ...
        """
        for fname, cols, table_attr in zip([uf_file, cl_file],
                                           [self.uf_columns, self.cl_columns],
                                           ['uf_table', 'cl_table']):
...
# TODO:
# - allow for appending more data rather than overwriting self.data_frame
# - have option to use self.uf_file, etc in addition to user-provided filenames
    def process_events(self, uf_file='evt0', cl_file='evt2', status_name='status'):
        ...
        table into a Pandas dataframe, stored in self.data_frame.

        Parameters
        ----------
        expand_status: bool
            Whether or not to expand the "status" field into a bunch of fields
            (since "status" is usually an array)
        """
...
# TODO: check for whether this actually is a MaskedColumn
        self.full_table.add_column(table.Column(np.int16(self.full_table['pha_cl'].mask)),
                                   name='bad_event')
        ...
        self.data_frame = self.full_table.to_pandas()
...
# This is len(self.data_frame.columns)-1 because the 'bad_event' column will be the label
...
# sequential dense model. Otherwise use the model provided by the user.
    def compile_model(self, dense_layers=(32,), model=None, optimizer='adam',
                      loss='binary_crossentropy', metrics=DEFAULT_METRICS):
...
# Just runs sklearn's train_test_split on self.data_frame and converts the
# resulting train/val/test dataframes to TensorFlow datasets
    def train_test_split(self, test_frac=0.2, val_frac=0.2):
        ...
        Splits self.data_frame into training, testing, and validation DataFrames and
        ...
        """
        self.train, self.test = train_test_split(self.data_frame, test_size=test_frac)
...
# - check if the model actually exists
    def fit(self, train_data=None, val_data=None, epochs=15):
        ...
        """
        if train_data is None and not hasattr(self, "train_data"):
